...
type handlerConfig struct {
	...
	goNum     int           // 需要启动的 goroutine 的数量
	number    int           // 单个 goroutine 中的处理次数
	interval  time.Duration // 单个 goroutine 中的处理间隔时间
	counter   int           // 数据量计数器，以字节为单位
	counterMu sync.Mutex    // 数据量计数器的专用的互斥锁
}
	...
	for i := 1; i <= writingConfig.goNum; i++ {
		go func(i int) {
			...
			for j := 1; j <= writingConfig.number; j++ {
				...
				if err != nil {
					log.Printf("writer [%d-%d]: errors: %s", i, j, err)
					continue
				}
